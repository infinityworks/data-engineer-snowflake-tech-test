...
class Customer(object):
    def __init__(self, customer_id, customer_name, loyalty_score):
...
def generate_customers(
    ...
    with open(
        ...
        csv_writer.writerow(["customer_id", "customer_name", "loyalty_score"])
        for cid in range(1, number_of_customers + 1):
            score = np.random.randint(low=1, high=11)
            customer_id = f"C{cid}"
            customer_name = names.get_full_name()
            csv_writer.writerow([customer_id, customer_name, score])
            if return_data:
                customers.append(Customer(customer_id, customer_name, score))
